...
public class CardLabelsService : ICardLabelsService
{
    ...
    public async Task<Result<List<CardLabelModel>>> GetByCardAsync(int cardId)
    {
        var response = await _cardLabelsApi.GetByCardAsync(cardId);

        return response.HandleResultWithMapping(content 
            => _mapper.Map<List<CardLabelModel>>(content));
    }

    public async Task<Result<CardLabelModel>> CreateAsync(CreateCardLabelRequest request)
    {
        var response = await _cardLabelsApi.CreateAsync(request);
    ...
    {
        var response = await _cardLabelsApi.DeleteAsync(id);
